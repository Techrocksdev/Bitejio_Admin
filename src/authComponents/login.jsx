...
function Login() {
  ...
  useEffect(() => {
    if (
      localStorage.getItem("token-bit-admin") &&
      localStorage.getItem("rememberMe-bit-admin")
    ) {
  ...
  const onSubmit = async (data) => {
    ...
    data.deviceId = localStorage.getItem("uid-bit-admin");
    data.deviceOS = "web";
    try {
      const response = await userLogin(data);

      if (!response.error) {
        localStorage.setItem("token-bit-admin", response.results.token);
